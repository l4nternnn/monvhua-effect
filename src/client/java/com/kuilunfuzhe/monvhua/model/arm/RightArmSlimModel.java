...
/**
 * 右臂Slim模型（手臂3像素宽），与{@link RightArmModel}的区别仅在于手臂宽度（3 vs 4像素）。
 *
 * <p>继承{@link SkullBlockEntityModel}是为了复用头颅方碑的渲染管线。
 * 同样使用{@link EntityModelPartNames#HEAD}作为子节点名来复用父类的旋转逻辑（详见{@link RightArmModel}）。</p>
 */
public class RightArmSlimModel extends SkullBlockEntityModel {
    private final ModelPart right_arm;

    public RightArmSlimModel(ModelPart root) {
        super(root);
        // 用HEAD常量名查找实际代表右臂的模型部件，以复用父类SkullBlockEntityModel的旋转方法
        this.right_arm = root.getChild(EntityModelPartNames.HEAD);
    }

    /** @return Slim体型右臂（3像素宽）的TexturedModelData */
    public static TexturedModelData getTexturedModelData() {
        ModelData modelData = new ModelData();
        ModelPartData root = modelData.getRoot();

        // 子节点名使用HEAD（而非RIGHT_ARM），使父类setHeadRotation能通过HEAD查找到此部件并旋转
        // Slim体型：3像素宽（比默认体型窄1像素）
