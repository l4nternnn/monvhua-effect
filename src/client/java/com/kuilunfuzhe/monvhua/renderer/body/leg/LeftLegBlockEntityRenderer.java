...
/**
 * 左腿方块实体渲染器。
 * 负责在方块世界中渲染玩家左腿模型，根据方块朝向自动旋转，
 * 从玩家Profile或UUID获取皮肤纹理，并优先使用体素渲染器渲染裤层。
 */
public class LeftLegBlockEntityRenderer implements BlockEntityRenderer<LeftLegBlockEntity> {
    ...
    /**
     * 渲染左腿方块实体。
     * 渲染流程：放置变换矩阵 → 获取皮肤纹理 → 先渲染内层（腿部主体）→ 再渲染外层（裤层/体素）
     *
     * @param entity 左腿方块实体
    ...
    public void render(LeftLegBlockEntity entity, float tickDelta, MatrixStack matrices,
        ...
        // Y轴向上偏移0.375：使腿模型在方块内垂直居中（腿模型高度为12像素，方块16像素，偏移=4/32*3=0.375）
