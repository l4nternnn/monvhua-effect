...
public final class SwingCommand {
    ...
    public static void register(CommandDispatcher<ServerCommandSource> d, net.minecraft.registry.RegistryWrapper.WrapperLookup r, CommandManager.RegistrationEnvironment e) {
        d.register(CommandManager.literal("monvhua-swing")
                ...
                .then(CommandManager.literal("stats").executes(c -> {
                    SwingEntity swing = nearest(c.getSource());
                    if (swing == null) return 0;
                    c.getSource().sendFeedback(() -> Text.literal("Swing " + swing.getUuid()
                            + " blocks=" + swing.structure().blocks().size()
                            + " collisionBoxes=" + swing.cachedCollisionBoxCount()
                            + " collisionBuilds=" + swing.collisionBuilds()
                            + " bounds=" + swing.structureBounds()), false);
                    return 1;
                }))
    ...
    private static SwingEntity nearest(ServerCommandSource source) {
        ...
        java.util.List<SwingEntity> entities = SwingSpatialIndex.find(w, box);
        return entities.stream().min(java.util.Comparator.comparingDouble((SwingEntity e) -> e.structureBounds().squaredMagnitude(p))).orElse(null);
    }
